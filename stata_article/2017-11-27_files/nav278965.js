...
i.__replaySec=null),i.__canplay=!0,i.hideCover(),i._trigger("canplay");
...
s.find(".js_switch")),f=s.find(".js_progress_bar"),g=s.find(".js_played_bar"),v=s.find(".js_page_video"),y=s.find(".js_full_mask"),w=s.find(".js_video_pause_controll"),m=s.find(".js_full_screen_control"),T=s.find(".js_loading");
w.on("touchend",".js_btn_pause",function(){
t.__forcePause=!1,t.play(),t._trigger("userplay"),t._hidePlayControllBar();
});
var C,j,P,b,B=0,S=!1,F=0,H=0,x=0,D=t.__getDuration(),k=0,M=1,V=top.window.user_uin||0,E=0!==V&&Math.floor(V/100)%1e3<M,L=!1,I=0;
...
if(parent.document.body.scrollTop!=x)return S=!1,void(x=-1);
var i=new Date,o=e.changedTouches[0].clientX,a=e.changedTouches[0].clientY,r=i-C,s=o-j.x,l=a-j.y,d=Math.sqrt(Math.pow(s,2)+Math.pow(l,2))+H,_=Math.min(Math.ceil(d/r),6);
F=Math.floor(.1*d+.2*_*_*_)*Math.ceil(D/500),H=0==F?d:0,0>s&&(F=-F);
var h=180*Math.atan2(l,s)/Math.PI;
S||(h>=-30&&30>=h&&++B,(h>=150&&180>=h||h>=-180&&-150>=h)&&--B,(B>=4||-4>=B)&&(5>=d?B=0:(I=Math.max(I,_),
S=!0))),S&&(k+=F,0>k&&(k=0),k>D&&(k=1*D),t.__setForwardBar(k),e.preventDefault(),
e.stopPropagation()),j={
x:o,
y:a
},C=i;
...
if(t.isEnd()||t.isPause()||S||t._trigger("touchVideo"),S&&(t.play(k),s.find(".js_forward").css("display","none"),
E&&((new Image).src="/mp/jsmonitor?idkey=28307_29_1",!L))){
var i=(new Date,{
...
(new Image).src="/mp/jsmonitor?idkey=28307_31_1;28307_33_"+a+";28307_34_"+I,L=!0;
}
F=0,S=!1,B=0;
